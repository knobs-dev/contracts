...
contract ArraysToGo {
    ...
    event ArrayCompleted(string indexed name, uint256 indexed limit, bytes32 indexed hash);
    ...
    function initialize(string memory arrayName, uint256 limit) public {
        require(limit > 0, "Not possible to register arrays with zero elements");
    ...
    function fillArrayByName(string memory arrayName, uint256[] memory data) public onlyCreator(arrayName) {
        require(data.length <= _arrays[arrayName].limit - _arrays[arrayName].data.length, "To much data");
        ...
        if(_arrays[arrayName].data.length == _arrays[arrayName].limit) {
            bytes32 arrayHash = keccak256(abi.encodePacked(_arrays[arrayName].data));
    ...
    function fillArrayByNameProgressively(string memory arrayName, uint256 numberOfElements) public onlyCreator(arrayName) onlyIfNotReady(arrayName) {
        uint256 start = _arrays[arrayName].data.length;
        uint256 remaining = _arrays[arrayName].limit - _arrays[arrayName].data.length;
    ...
    function getArrayByName(string memory arrayName) public view returns (uint256[] memory) {
        return _arrays[arrayName].data;
    }

    function safeGetArrayByName(string memory arrayName) public view onlyIfReady(arrayName) returns (uint256[] memory)   {
        return _arrays[arrayName].data;
    }

    function getArrayInfo(string memory arrayName) public view returns(address, uint256, bool, bytes32, uint256) {
        return (_arrays[arrayName].creator, _arrays[arrayName].limit, _arrays[arrayName].ready, _arrays[arrayName].hash, _arrays[arrayName].data.length);
    }
}